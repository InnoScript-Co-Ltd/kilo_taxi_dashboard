...
import { ExtraDemandFormInputs, extraDemandSchema } from "../extraDemand.payload"; // Assuming walletSchema for validation
...
const ExtraDemandUpdate = () => {
  ...
  const { extraDemand } = useSelector((state: AppRootState) => state.extraDemand); // Selecting wallet data from the store
          ...
          <Grid2 container spacing={2}>
            <Grid2 size={{ xs: 6, md: 3 }}>
              <FormControl variant="filled" fullWidth error={!!errors.title}>
                <InputLabel htmlFor="ExtraDemand_name">ExtraDemand Name</InputLabel>
                <FilledInput
                  size="small"
                  id="ExtraDemand_name"
                  {...register("title")}
