...
export const scheduleBookingService = {
    ...
    store: async (payload: ScheduleBookingFormInputs, dispatch: Dispatch, notifications? : any) => {
        const response: any = await postRequest(endpoints.scheduleBooking, payload, dispatch);
    ...
    index: async (dispatch: Dispatch, params: any, notifications? : any) => {
        const response: any = await getRequest(endpoints.scheduleBooking, params, dispatch);
        await httpServiceHandler(dispatch, response);
        if(response.status === 200) { 
    ...
    update: async (dispatch: Dispatch, id: number, payload: ScheduleBookingFormInputs, notifications? : any) => {
        const response: any = await putRequest(`${endpoints.scheduleBooking}/${id}`, payload, dispatch);
        await httpServiceHandler(dispatch, response);
    ...
    show: async (dispatch: Dispatch, id: number) => {
        const response: any = await getRequest(`${endpoints.scheduleBooking}/${id}`, null, dispatch);
        await httpServiceHandler(dispatch, response);
